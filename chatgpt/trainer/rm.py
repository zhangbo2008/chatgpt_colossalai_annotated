    ...
    def fit(self, use_lora):
        ...
        for epoch in range(self.epochs):
            ...
            for chosen_ids, c_mask, reject_ids, r_mask in self.train_dataloader:
                chosen_ids = chosen_ids.squeeze(1).cpu()
                c_mask = c_mask.squeeze(1).cpu()
                reject_ids = reject_ids.squeeze(1).cpu()
                r_mask = r_mask.squeeze(1).cpu()
                chosen_reward = self.model(chosen_ids, attention_mask=c_mask)
                reject_reward = self.model(reject_ids, attention_mask=r_mask)
                loss = self.loss_fn(chosen_reward, reject_reward)#=========loss_fn核心看这个函数就行了.
            ...
            with torch.no_grad():
                ...
                for chosen_ids, c_mask, reject_ids, r_mask in self.eval_dataloader:
                    ...
                    chosen_reward = self.model(chosen_ids, attention_mask=c_mask)
                    reject_reward = self.model(reject_ids, attention_mask=r_mask)
                    dist += (chosen_reward - reject_reward).mean().item()#==========两个平分的差
