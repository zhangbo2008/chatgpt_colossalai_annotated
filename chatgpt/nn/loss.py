...
class PairWiseLoss(nn.Module):
    ...
    def forward(self, chosen_reward: torch.Tensor, reject_reward: torch.Tensor) -> torch.Tensor:
        probs = torch.sigmoid(chosen_reward - reject_reward)#=======核心loss, 神将网络给正样本打的分, 剪去给负样本打得分. 用sigmoid函数让他得到0,1之间的值.
        log_probs = torch.log(probs)#越大越好就行了.所以睁眼本趋近于无穷,负样本趋近于负无穷.
        loss = -log_probs.mean()
        return loss
